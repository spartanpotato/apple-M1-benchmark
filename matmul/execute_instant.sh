...
for (( p = start_power; p <= end_power; p++ )); do
  ...
  sudo ./prog --n $n --hardware $hardware --precision $precision --check 0 --instantEnergy 1 --energyOverTime 0 --iterations $iterations
done

# Transform data
echo "Transforming data with hardware=$hardware, precision=$precision"
python ./python_scripts/transform_${hardware}_instant.py ${precision}
